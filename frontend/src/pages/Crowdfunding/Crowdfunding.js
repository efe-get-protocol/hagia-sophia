  ...
  if(researches) return (
    <div>
      ...
      <div className="info-container">
        {researches.map(item => <Infographic image={item.image} organization={item.title} description={item.description} amount_raised={item.fundingReceived} target_amount={item.fundingLimit} />
          )}
      </div>
    </div>
  )
